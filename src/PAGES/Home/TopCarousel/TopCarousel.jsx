                        ...
                        return (
                            <div key={meal?._id} className="p-2 bg-base-200 relative">
                                ...
                                <div className="custom-w p-2 absolute bottom-5 left-10 sm:left-4 w-10/12 sm:w-11/12 h-1/4 hover:h-3/4 sm:hover:h-4/5 lg:hover:h-4/5 transform transition-all bg-white rounded-md overflow-hidden">
                                    ...
                                    <Link to={`/meal/${meal?._id}`}><h1 className='set-text text-1xl sm:font-bold lg:font-normal sm:text-md md:text-2xl md:my-6 lg:text-2xl my-4'>{meal?.title}</h1></Link>

                                    <div className='mt-5'>
                                        <p className='custom-hide-description text-sm'>
                                            {meal?.description.length < 150 ? <>{meal?.description}</> : <>{meal?.description.slice(0, 150)}...<Link className='text-blue-600 font-bold underline' to={`/meal/${meal?._id}`}>read more</Link></>}
                                        </p>
                                        ...
                                        <div className="flex items-center gap-4 mt-4 sm:mt-0 md:mt-3 pr-5">
                                            <Link to={`/meal/${meal?._id}`} className='w-2/12'><img className='rounded-full' src={meal?.authorImage} alt="" /></Link>
